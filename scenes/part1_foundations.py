...
class Slide11_WooclapPlaceholder(LLMSlide):
    """
    Slide 11: Wooclap Placeholder
    ...
    def construct(self):
        ...
        box_title.move_to(main_box.get_top() + DOWN * 0.8)

        # Instructions
        instructions = [
            "1. Install the Chrome or Firefox extension 🧩",
            "2. Click on 'Add a Wooclap vote' and make sure\n   you are logged into your Wooclap account 🏠",
            "3. Ensure you are in presentation mode\n   on Google Slides 📊"
        ]

        instruction_objects = VGroup()
        for instr in instructions:
            text = Text(instr, font_size=SMALL_FONT_SIZE, color=WHITE, line_spacing=1.2)
            instruction_objects.add(text)

        instruction_objects.arrange(DOWN, aligned_edge=LEFT, buff=0.3)
        instruction_objects.move_to(main_box.get_center() + DOWN * 0.3)

        # Animate
        self.play(Create(main_box), run_time=0.5)
        self.wait(0.2)
        self.play(FadeIn(box_title), run_time=0.5)
        self.wait(0.3)

        for instr in instruction_objects:
            self.play(FadeIn(instr, shift=RIGHT), run_time=0.4)
            self.wait(0.1)
